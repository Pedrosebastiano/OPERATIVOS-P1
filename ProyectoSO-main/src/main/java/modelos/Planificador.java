...
public class Planificador {
    ...
    public Planificador(List readyList, List blockedList, List exitList, List allProcess, 
        ...
        this.memoryManager = new MemoryManager(200); 
        
        // Calculate initial memory usage
        calculateInitialMemory();
        logger.logEvent("Sistema iniciado. Memoria disponible: " + memoryManager.getAvailableMemory() + " MB");
    }

    private void calculateInitialMemory() {
        Nodo current = readyList.getHead();
        while (current != null) {
    ...
    public Proceso getProcess(){
        ...
        // Check memory and try to resume suspended processes
        ...
        if(this.readyList.isEmpty()){
            ...
            // Memory should already be allocated, but verify
    ...
    /**
     * Check if there's enough memory and resume suspended processes
     * Based on JuanGzlz's checkMemoryAndResume implementation
     */
    private void checkMemoryAndResume() {
        // Try to reactivate suspended-ready processes first
    ...
    /**
     * Check memory and suspend processes if memory is low
     * Based on JuanGzlz's checkMemoryAndSuspend implementation
     */
    private void checkMemoryAndSuspend() {
        if (memoryManager.getAvailableMemory() < LOW_MEMORY_THRESHOLD) {
            ...
            // If still low, suspend blocked processes
    ...
    private void handleStateTransition(Proceso process, String state) {
        switch (state) {
            case "Bloqueado":
                // Check if we need to suspend due to low memory
    ...
    public void updateWaitingTime(){
        ...
        // Also update waiting time for suspended ready processes
    ...
    public void updateBlockToReady(int id){
        ...
        // Check in suspended blocked list
